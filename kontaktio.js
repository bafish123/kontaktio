(function () {
  ...
  #k-launcher:hover{transform:scale(1.07);box-shadow:0 12px 30px rgba(0,0,0,.35)}
  ...
  #k-widget.open{opacity:1;transform:none;pointer-events:auto}
  ...
  #k-messages{flex:1;padding:12px;overflow-y:auto}
  ...
  .k-bot{background:var(--k-bot);color:var(--k-text)}
  .k-typing{font-size:12px;opacity:.6;animation:pulse 1.2s infinite}
  @keyframes pulse{0%{opacity:.3}50%{opacity:.8}100%{opacity:.3}}

  #k-input{display:flex;border-top:1px solid rgba(0,0,0,.1)}
  #k-input input{
  ...
  launcher.id="k-launcher";launcher.textContent="💬";
  ...
  async function send(){
    ...
    try{
      ...
      if(!themeApplied&&data.theme){
        ...
        Object.entries(data.theme).forEach(([k,v])=>r.setProperty(`--k-${k}`,v));
        themeApplied=true;
      }
  ...
  launcher.onclick=()=>{isOpen=!isOpen;widget.classList.toggle("open",isOpen);if(isOpen)input.focus()};
  closeBtn.onclick=()=>{isOpen=false;widget.classList.remove("open")};
  themeBtn.onclick=()=>{darkMode=!darkMode;document.body.classList.toggle("k-dark",darkMode);localStorage.setItem("kontaktio-dark",darkMode?"1":"0")};
  button.onclick=send;
